...
trait WebBotReplyTrait
{
    use CommonTrait, WhatsAppWebTrait;
    ...
    public function QuickReply($message, $token)
    {
        DB::beginTransaction();
        try {
            $flow = null;
            ...
            if ($currentFlowState) {
                ...
                $this->flowReply($flow_id, $contact, $client, $message, $token);
            } elseif (!empty($flow)) {
                $this->flowReply($flow, $contact, $client, $message, $token);
            } else {
                $this->BOTReply($message, $contact, $client, $token);
            }
    ...
    public function BOTReply($message, $contact, $client, $token)
    {
        if (empty($message)) {
            return null;
        }
        ...
        try {
            ...
            if ($message_text) {
                ...
                $this->sendWhatsAppWebMessage($reply_message, $message_type, $context = null, $token->whatsapp_session);
            }
        } catch (\Exception $e) {
    ...
    private function flowReply($flowId, $contact, $client, $message, $token)
    {
        try {
            ...
            if (isset($next_node) && $next_node['node_id']) {
                ...
                if (!empty($reply)) {
                    ...
                    $this->sendWhatsAppWebMessage($reply, $reply->message_type, $context = null, $token->whatsapp_session);;
                }
                if (
                    ...
                    $this->flowReply($flowId, $contact, $client, $reply, $token);
                }
                return true;
            }
            return true;
